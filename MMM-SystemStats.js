...
Module.register('MMM-SystemStats', {
  ...
  start: function() {
    ...
    this.stats.wifi = this.translate('LOADING').toLowerCase();
  ...
  socketNotificationReceived: function(notification, payload) {
    ...
    if (notification === 'STATS') {
      ...
      sysLoad = Number(payload.sysLoad)
      this.stats.sysLoad = (sysLoad).toFixed(1) + '%';
      this.stats.freeMem = Number(payload.freeMem).toFixed() + '%';
      wifi = Number(payload.wifi)
      this.stats.wifi = (wifi*100/70).toFixed() +'%';
  ...
  getDom: function() {
    ...
    wrapper.innerHTML = '<tr>' +
                        ...
                        '</tr>' +
                        '<tr>' +
                        '<td class="title" style="text-align:' + self.config.align + ';">' + this.translate("CPU Load") + ':&nbsp;</td>' +
                        ...
                        '</tr>' +
			'<tr>' +
			'<td class="title" style="text-align:' + self.config.align + ';">' + this.translate("WiFi Quality") + ':&nbsp;</td>' +
			'<td class="value" style="text-align:left;">' + this.stats.wifi + '</td>' +
                        '</tr>' +
                        '<tr>' +
